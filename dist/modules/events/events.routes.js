...
const eventSchema = zod_1.z.object({
    ...
    opening_time: zod_1.z.string().optional(),
    match_time: zod_1.z.string().optional(),
...
router.post("/", auth_1.authMiddleware, (0, roles_1.requireRoles)("CHEF_OPS", "ADMIN"), async (req, res) => {
    const schema = zod_1.z.object({
        ...
        match_time: zod_1.z.string().optional(),
        notes: zod_1.z.string().optional(),
        buvetteIds: zod_1.z.array(zod_1.z.number()).optional(),
    });
    ...
    if (body.buvetteIds && body.buvetteIds.length > 0) {
        await prisma_1.prisma.eventBuvette.createMany({
            data: body.buvetteIds.map((buvetteId) => ({
                eventId: event.id,
                buvetteId
            }))
        });
...
router.get("/:id/buvettes", async (req, res) => {
    const id = Number(req.params.id);
    const links = await prisma_1.prisma.eventBuvette.findMany({
        where: { eventId: id },
...
router.post("/:id/buvettes", auth_1.authMiddleware, (0, roles_1.requireRoles)("CHEF_OPS"), async (req, res) => {
    const id = Number(req.params.id);
    const schema = zod_1.z.object({
    ...
    if (!parsed.success)
        return res.status(400).json({ message: parsed.error.message });
    const link = await prisma_1.prisma.eventBuvette.upsert({
...
router.get("/:id/timesheets", async (req, res) => {
    ...
    if (!buvetteId)
        return res.status(400).json({ message: "buvetteId is required" });
    const timesheet = await prisma_1.prisma.timesheet.findFirst({
        where: {
            eventId,
            buvetteId
        },
...
router.post("/:id/timesheets", async (req, res) => {
    ...
    const eventBuvette = await prisma_1.prisma.eventBuvette.findUnique({
        where: {
            eventId_buvetteId: {
                eventId,
...
router.get("/:eventId/buvettes/:buvetteId/inventory", async (req, res) => {
    ...
    const buvetteProducts = await prisma_1.prisma.buvetteProduct.findMany({
        where: { buvetteId },
        include: { product: true },
        orderBy: { displayOrder: 'asc' }
    });
    // 2. Get existing inventory deltas (main data source now)
    const deltas = await prisma_1.prisma.inventoryDelta.findMany({
...
router.post("/:eventId/buvettes/:buvetteId/inventory", async (req, res) => {
    ...
    const schema = zod_1.z.object({
        items: zod_1.z.array(zod_1.z.object({
            ...
            initialQty: zod_1.z.number().nullable().optional(),
            finalQty: zod_1.z.number().nullable().optional(),
            restockQty: zod_1.z.number().nullable().optional(),
            isRestocked: zod_1.z.boolean().optional(),
            hasShortage: zod_1.z.boolean().optional(),
            shortageQty: zod_1.z.number().nullable().optional()
        }))
    });
    const parsed = schema.safeParse(req.body);
    if (!parsed.success)
    ...
    const results = await prisma_1.prisma.$transaction(parsed.data.items.map(item => prisma_1.prisma.inventoryDelta.upsert({
        where: {
...
router.get("/:eventId/shortages", async (req, res) => {
    const eventId = Number(req.params.eventId);
    const shortages = await prisma_1.prisma.inventoryDelta.findMany({
        where: {
            eventId,
            hasShortage: true
        },
...
router.get("/:eventId/timesheets/summary", auth_1.authMiddleware, async (req, res) => {
    const eventId = Number(req.params.eventId);
...
router.get("/:eventId/inventory/summary", auth_1.authMiddleware, async (req, res) => {
    const eventId = Number(req.params.eventId);
    const deltas = await prisma_1.prisma.inventoryDelta.findMany({
        where: { eventId },
        include: {
    ...
    for (const d of deltas) {
        ...
        if (d.isRestocked)
            byBuvette[d.buvetteId].restockedCount += 1;
        if (d.hasShortage)
            byBuvette[d.buvetteId].shortageCount += 1;
...
router.get("/:eventId/checklists/summary", auth_1.authMiddleware, async (req, res) => {
    const eventId = Number(req.params.eventId);
    const checklists = await prisma_1.prisma.checklistResponse.findMany({
        where: { eventId },
        include: {
            buvette: true,
            template: true,
...
router.get("/:eventId/timesheets/detailed", auth_1.authMiddleware, async (req, res) => {
    const eventId = Number(req.params.eventId);
    // Get all buvettes for this event
    const eventBuvettes = await prisma_1.prisma.eventBuvette.findMany({
        where: { eventId },
        include: { buvette: true, responsable: true }
    });
...
router.get("/:eventId/hours/export", auth_1.authMiddleware, async (req, res) => {
    const eventId = Number(req.params.eventId);
    // Get event info
    const event = await prisma_1.prisma.event.findUnique({ where: { id: eventId } });
    if (!event)
    ...
    rows.sort((a, b) => {
        if (a.buvette !== b.buvette)
            return a.buvette.localeCompare(b.buvette);
